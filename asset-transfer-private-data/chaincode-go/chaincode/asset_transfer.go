...
// Asset describes main data details that are visible to Users of the application (not researchers)
...
// UserDataAppraisalDetails describes details that are private to owners of the data
type UserDataAppraisalDetails struct {
	Type           string `json:"objectType"`
	ID             string `json:"assetID"`
	AppraisedValue int    `json:"appraisedValue"`
}
...
// CreateAsset creates a new asset by placing the asset details in the public assetCollection
// that can be read by peers in public organization.
func (s *SmartContract) CreatePublicAsset(ctx contractapi.TransactionContextInterface) error {
	...
	if len(assetInput.Provider) == 0 {
		return fmt.Errorf("provider field must be a non-empty string")
	}
	if len(assetInput.AgeRange) == 0 {
		return fmt.Errorf("AgeRange field must be a non-empty string")
	}
	if len(assetInput.Data) == 0 {
		return fmt.Errorf("no Data Provided")
	}

	log.Printf("assetInput:  %v", assetInput)
	...
	asset := PublicAsset{
		...
		AgeRange: assetInput.AgeRange,
		Data:     assetInput.Data,
	}
	log.Printf("assetPublic:  %v", asset)
	assetJSONasBytes, err := json.Marshal(asset)
	if err != nil {
		return fmt.Errorf("failed to marshal asset into JSON: %v", err)
	}
...
func (s *SmartContract) setPrivateAssetPrice(ctx contractapi.TransactionContextInterface) error {
	...
	// Check if asset already exists
	assetAsBytes, err := ctx.GetStub().GetPrivateData(orgCollection, assetInput.ID)
	if err != nil {
		return fmt.Errorf("failed to get asset: %v", err)
	} else if assetAsBytes != nil {
		fmt.Println("Asset already exists: " + assetInput.ID)
	...
	// Save asset details to collection visible to owning organization
	assetPrivateDetails := UserDataAppraisalDetails{
		ID:             assetInput.ID,
		AppraisedValue: assetInput.AppraisedValue,
		Type:           assetInput.Type,
	}
